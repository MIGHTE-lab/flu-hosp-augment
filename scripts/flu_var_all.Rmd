...
write_csv(retro_predictions, file = 'flu_var_predictions_all.csv', append = F)
```
...
write_csv(future_predictions, file = 'flu_var_predictions_all.csv', append = T)
```
...
write_csv(retro_predictions, file = 'flu_var_predictions_all.csv', append = T)
```
...
df <- read_csv('flu_var_predictions_all.csv') %>%
  arrange(location_name, date_predicted)

write_csv(df, file = 'flu_var_predictions_all.csv')
...
cols <- c('red', 'black')
p <- ggplot(df, 
            aes(x = date_predicted, y = values, color = series, fill = series)) + 
  geom_line(size = 1.0,
            alpha = 0.6) +
